...
void showHeader(struct Header *hdr) {
    printf("[0x%08x-0x%08x], entry 0x%08x", hdr->minAddr, hdr->maxAddr,
           hdr->entry);
}
 ...
 */
struct Header *addHeader(struct Header *hdrs, unsigned minAddr,
                         unsigned maxAddr, unsigned entry) {
    ...
    if (stat(filename, &sb)) {
        ...
        if (!(contents = (char *)malloc(length))) {
            printf("Could not allocate buffer for contents of \"%s\"\n",
                   filename);
            ABORT;
        } else if (!(img = (struct FileImage *)malloc(
                         sizeof(struct FileImage)))) {
...
struct ElfHeader *elfHeader(void *contents, unsigned length) {
    ...
    if (length >= sizeof(struct ElfHeader) && hdr->ident[0] == 0x7f &&
        hdr->ident[1] == 'E' && hdr->ident[2] == 'L' && hdr->ident[3] == 'F' &&
 ...
 */
struct Section *section(unsigned first, unsigned last, struct FileImage *img,
                        unsigned offset) {
...
void checkEntry(struct MemImage *mimg) {
    ...
    for (; list; list = list->next) {
        if (list->img && list->first <= mimg->entry &&
            mimg->entry <= list->last) {
            return;
        }
    }
            ...
            for (; i < phnum; i++) {
                if (elfWord(hdr, phdrs[i].type) == PT_LOAD) {
                    ...
                    if (offset + filesz > img->length) {
                        printf(
                            "Invalid ELF section passes end of file \"%s\"\n",
                            filename);
                        ABORT;
                    }
                    ...
                    if (load) {
                        if (filesz > 0) {
                            insert(mimg, section(paddr, paddr + filesz - 1, img,
                                                 offset));
                        }
                        if (memsz > filesz) {
                            insert(mimg,
                                   section(paddr + filesz, paddr + memsz - 1,
                                           NULL, ZERO));
                        }
                    } else {
                        insert(mimg, section(paddr, paddr + memsz - 1, NULL,
                                             RESERVED));
                    }
                    if (paddr < minAddr) {
                        minAddr = paddr;
...
static char *readAddr(char *arg, char *s) {
    ...
    addr = 0;
    if (s[0] == '0' &&
        (s[1] == 'x' || s[1] == 'X')) { /* skip leading 0x prefix */
        s += 2;
    }
...
int main(int argc, char *argv[]) {
    ...
    if (argc < 2) {
        ...
        printf(
            "  reserved:addr-addr reserve all addresses in specified range\n");
