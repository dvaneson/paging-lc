 ...
 */
struct MultibootMMap defaultMMap[2] = {{20, 0, 0, 0, 0, 1},
                                       {20, 0, 0, 0, 0, 1}};
...
int mmapAvailable(struct MultibootMMap *mmap) {
    return (mmap->type = 1) && (mmap->baseHi == 0) && (mmap->lenHi == 0) &&
...
int fitsInMemory(unsigned first, unsigned last) {
    ...
    while (m < l) {
        ...
        if (mmap->baseLo <= first && last < mmap->baseLo + mmap->lenLo &&
            mmapAvailable(mmap)) {
            return 1;
        }
        ...
        while (start <= finish) {
            ...
            if (start + sizeof(struct SectionHeader) > finish + 1) {
                ...
                return "section overlaps with loader";
                       ...
                       (curr->first + BOOTLEN(*(unsigned *)(curr + 1))) >
                           curr->last + 1) {
